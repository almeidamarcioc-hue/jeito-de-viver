...
import { Agendamento, Pastor, Configuracoes } from '@/types'
...
export default function GerenciarAgendaPage() {
  ...
  const selecionar = (ag: Agendamento) => {
    setSelecionado(ag)
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/agendamentos/${selecionado.id}`, {
        method: 'PUT',
      ...
      if (status === 'cancelado' && oldStatus !== 'cancelado') {
        const msg = config?.msg_cancelamento || `Olá ${nomeFiel}, seu agendamento foi cancelado.`
        const msgFmt = msg.replace('{nome_fiel}', nomeFiel).replace('{data}', data).replace('{hora}', hora)
        abrirWhatsApp(telefone, msgFmt)
      }

      // WhatsApp remarcação
      if ((data !== oldData || hora !== oldHora) && status !== 'cancelado') {
        const msg = config?.msg_remarcacao || `Olá ${nomeFiel}, seu agendamento foi remarcado para ${data} às ${hora}.`
        const msgFmt = msg.replace('{nome_fiel}', nomeFiel).replace('{data}', data).replace('{hora}', hora)
        abrirWhatsApp(telefone, msgFmt)
      }
  ...
  const handleExcluir = async () => {
    ...
    try {
      const res = await fetch(`/api/agendamentos/${selecionado.id}`, { method: 'DELETE' })
    ...
    <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <input
              type="date"
              value={filtroB}
              onChange={(e) => setFiltroB(e.target.value)}
              className="w-full border rounded-lg px-2 py-1.5 text-sm focus:outline-none"
              style={{ borderColor: '#e5e7eb' }}
            />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          <h2 style={{ color: '#002347' }} className="font-bold mb-3 text-sm">
            ...
            <div className="overflow-y-auto max-h-[60vh] space-y-1">
              {agendamentos.map((ag) => (
                <button
                  ...
                  style={{
                    borderColor: selecionado?.id === ag.id ? '#C5A059' : '#e5e7eb',
                    backgroundColor: selecionado?.id === ag.id ? '#fffbea' : undefined,
                  }}
                >
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          {!selecionado ? (
            ...
            <form onSubmit={handleSalvar} className="space-y-3">
              <h2 style={{ color: '#002347' }} className="font-bold text-sm mb-2">
                Editando: {selecionado.nome_fiel}
              </h2>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full border rounded-lg px-2 py-1.5 text-sm focus:outline-none resize-none"
                  style={{ borderColor: '#e5e7eb' }}
                />
              </div>
