...
function formatDateLabel(d: Date, view: string): string {
  ...
  if (view === 'semanal') {
    const ini = startOfWeek(d); const fim = addDays(ini, 6)
...
export default function AgendaPastoresPage() {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      ...
      if (view === 'mensal') {
        ...
        dataInicio = toDateStr(ini); dataFim = toDateStr(addDays(ini, 6))
      } else {
  ...
  const abrirPainel = (data: string, hora: string) => { setPainel({ data, hora, slot: getSlot(data, hora) }); setMotivoBloqueio(''); setPainelDia(null) }
  const abrirPainelDia = (ds: string) => { setPainelDia(ds); setPainel(null) }
  ...
  const handleBloquearDia = async (data: string) => {
    ...
    try {
      await fetch('/api/secretaria/bloqueios', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pastorId, data, hora: 'dia', motivo: motivoBloqueio || 'Dia bloqueado' }) })
  ...
  const handleDesbloquearDia = async (data: string) => {
    ...
    setBloqueandoDia(true)
    try {
      await fetch(`/api/secretaria/bloqueios?pastorId=${pastorId}&data=${data}`, { method: 'DELETE' })
      await fetchSlots(); setPainelDia(null)
    } finally { setBloqueandoDia(false) }
  }
  ...
  const weekDays = (): Date[] => { const ini = startOfWeek(currentDate); return Array.from({ length: 7 }, (_, i) => addDays(ini, i)) }
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-4 flex flex-wrap items-center gap-4">
        <div>
          ...
          <select value={pastorId ?? ''} onChange={e => setPastorId(e.target.value ? Number(e.target.value) : null)}
            className="border rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none" style={{ borderColor: '#e5e7eb', color: '#1a202c' }}>
        ...
        <div className="flex items-center gap-2 ml-auto flex-wrap">
          ...
          <button onClick={fetchSlots} style={{ backgroundColor: '#f3f4f6', color: '#374151' }} className="px-3 py-1.5 rounded-lg text-sm font-semibold">↻</button>
        </div>
      </div>

      {!pastorId && <div className="bg-white rounded-xl shadow-sm p-10 text-center text-gray-400"><p className="font-semibold text-gray-500">Selecione um pastor para visualizar a agenda</p></div>}
          ...
          {view === 'mensal' && (
            <div>
              <div className="grid grid-cols-7 gap-1 mb-2">
                {DIAS_SEMANA_PT.map(d => <div key={d} className="text-center text-xs font-bold text-gray-500 py-1">{d}</div>)}
              </div>
              <div className="grid grid-cols-7 gap-1">
                {daysInMonth().map((day, i) => {
                  ...
                  const agendamentos = Object.entries(daySlots).filter(([, s]) => s.tipo !== 'bloqueado').sort(([a], [b]) => a.localeCompare(b))
                  const bloqueios = Object.entries(daySlots).filter(([, s]) => s.tipo === 'bloqueado')
                  const isToday = ds === hoje
                  return (
                    <div key={ds} onClick={() => abrirPainelDia(ds)}
                      style={{ border: isToday ? '2px solid #C5A059' : '1px solid #e5e7eb', backgroundColor: isToday ? '#fffbf0' : '#fff', cursor: 'pointer', minHeight: 80 }}
                      className="rounded-lg p-1.5 hover:bg-blue-50 transition-colors">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-bold" style={{ color: isToday ? '#C5A059' : '#374151' }}>{day.getDate()}</span>
                        {(agendamentos.length + bloqueios.length) > 0 && (
                          <span className="text-xs font-semibold rounded-full px-1" style={{ backgroundColor: '#002347', color: '#C5A059', fontSize: 10 }}>{agendamentos.length + bloqueios.length}</span>
                        )}
                      </div>
                      <div className="space-y-0.5">
                        {agendamentos.slice(0, 3).map(([hora, slot]) => (
                          <div key={hora} className="rounded px-1 py-0.5 text-xs truncate leading-tight"
                            style={{ backgroundColor: slot.tipo === 'confirmado' ? '#dcfce7' : '#fef3c7', color: slot.tipo === 'confirmado' ? '#166534' : '#92400e' }}>
                            <span className="font-semibold">{hora}</span> {String(slot.dados.nome_fiel || '').split(' ')[0]}
                          </div>
                        ))}
                        {bloqueios.length > 0 && agendamentos.length < 3 && (
                          <div className="rounded px-1 py-0.5 text-xs truncate leading-tight" style={{ backgroundColor: '#f3f4f6', color: '#6b7280' }}>🔒 {bloqueios.length}</div>
                        )}
                      </div>
                    </div>
                  )
                ...
                <thead>
                  <tr>
                    ...
                    {weekDays().map(day => {
                      const ds = toDateStr(day)
                      return (
                        <th key={ds} className="py-2 font-semibold border-b text-center" style={{ color: ds === hoje ? '#C5A059' : '#374151' }}>
                          {DIAS_SEMANA_PT[day.getDay()]}<br /><span className="font-normal">{String(day.getDate()).padStart(2,'0')}/{String(day.getMonth()+1).padStart(2,'0')}</span>
                        </th>
                      )
                    })}
                  </tr>
                      ...
                      {weekDays().map(day => {
                        ...
                        return (
                          <td key={ds} className="py-1 px-1">
                            <button onClick={() => abrirPainel(ds, hora)}
                              ...
                              {slot ? (slot.tipo === 'bloqueado' ? '🔒' : String(slot.dados.nome_fiel || '').split(' ')[0] || '✓') : <span className="opacity-30">—</span>}
                            </button>
                          </td>
                        )
                      })}
          ...
          {view === 'diaria' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-3">
                <p style={{ color: '#002347' }} className="font-bold text-sm">{String(currentDate.getDate()).padStart(2,'0')}/{String(currentDate.getMonth()+1).padStart(2,'0')}/{currentDate.getFullYear()}</p>
                <div className="flex gap-2">
                  <button onClick={() => handleBloquearDia(toDateStr(currentDate))} disabled={bloqueandoDia} style={{ backgroundColor: '#002347', color: '#fff' }} className="px-3 py-1.5 rounded-lg text-xs font-semibold disabled:opacity-50">{bloqueandoDia ? '...' : '🔒 Bloquear Dia'}</button>
                  {Object.values(slots[toDateStr(currentDate)] || {}).some(s => s.tipo === 'bloqueado') && (
                    <button onClick={() => handleDesbloquearDia(toDateStr(currentDate))} disabled={bloqueandoDia} style={{ backgroundColor: '#ef4444', color: '#fff' }} className="px-3 py-1.5 rounded-lg text-xs font-semibold disabled:opacity-50">🔓 Desbloquear</button>
                  )}
                </div>
              </div>
              {HORAS_DIA.map(hora => {
                ...
                return (
                  <div key={hora} className="flex items-center gap-3">
                    ...
                    <button onClick={() => abrirPainel(ds, hora)} style={{ backgroundColor: cor.bg, borderColor: cor.border, color: cor.text }} className="flex-1 text-left px-4 py-2 rounded-lg border text-sm font-medium">
        ...
        <div className="fixed inset-0 z-50 flex" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onClick={e => { if (e.target === e.currentTarget) setPainelDia(null) }}>
          <div className="ml-auto w-96 bg-white h-full shadow-2xl flex flex-col overflow-y-auto">
            ...
            <div className="px-5 py-5 flex-1">
              {slotsDia.length === 0 ? <p className="text-green-600 font-semibold text-sm">✓ Nenhum agendamento neste dia</p> : (
                <div className="space-y-3">
                  {slotsDia.map(([hora, slot]) => {
                    const cor = slotColor(slot.tipo)
                    return (
                      <div key={hora} className="rounded-xl p-3 cursor-pointer hover:opacity-90" style={{ backgroundColor: cor.bg, border: `1px solid ${cor.border}` }} onClick={() => abrirPainel(painelDia, hora)}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-bold text-sm" style={{ color: cor.text }}>{hora}</span>
                          <Badge status={slot.tipo} />
                        </div>
                        {slot.tipo === 'bloqueado' ? <p className="text-xs text-gray-500">🔒 {String(slot.dados.motivo || 'Bloqueado')}</p> : (
                          <><p className="font-semibold text-sm" style={{ color: cor.text }}>{String(slot.dados.nome_fiel || '')}</p><p className="text-xs text-gray-500 mt-0.5">{String(slot.dados.assunto || '')}</p></>
                        )}
                      </div>
                    )
                  })}
            ...
            <div className="px-5 py-5 flex-1">
              ...
              {painel.slot?.tipo === 'bloqueado' && (
                <div>
                  ...
                  <button onClick={handleDesbloquear} style={{ backgroundColor: '#ef4444', color: '#fff' }} className="w-full py-2 rounded-lg font-semibold text-sm">🔓 Desbloquear</button>
                </div>
              )}
              {(painel.slot?.tipo === 'confirmado' || painel.slot?.tipo === 'pendente') && (
                <div>
                  <div className="flex items-center gap-2 mb-4"><Badge status={painel.slot.tipo} /></div>
                  ...
                  <div className="mt-5 space-y-2">
                    {painel.slot.tipo === 'pendente' && <button onClick={() => handleAlterarStatus('confirmado')} disabled={atualizandoStatus} style={{ backgroundColor: '#166534', color: '#fff' }} className="w-full py-2 rounded-lg text-sm font-semibold disabled:opacity-50">{atualizandoStatus ? '...' : '✓ Confirmar'}</button>}
                    {painel.slot.tipo === 'confirmado' && <button onClick={() => handleAlterarStatus('pendente')} disabled={atualizandoStatus} style={{ backgroundColor: '#92400e', color: '#fff' }} className="w-full py-2 rounded-lg text-sm font-semibold disabled:opacity-50">{atualizandoStatus ? '...' : '↩ Voltar para Pendente'}</button>}
                    <button onClick={() => handleAlterarStatus('cancelado')} disabled={atualizandoStatus} style={{ backgroundColor: '#ef4444', color: '#fff' }} className="w-full py-2 rounded-lg text-sm font-semibold disabled:opacity-50">🗑 Cancelar</button>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => {
                      const d = painel.slot!.dados
                      const [y,m,dv] = painel.data.split('-')
                      const templateData = { nome: String(d.nome_fiel||''), nome_fiel: String(d.nome_fiel||''), pastor: pastor?.nome||'', pastor_nome: pastor?.nome||'', data: `${dv}/${m}/${y}`, hora: painel.hora, assunto: String(d.assunto||''), telefone: String(d.telefone||'') }
                      const msg = config?.msg_lembrete ? preencherTemplate(config.msg_lembrete, templateData) : `Olá ${d.nome_fiel}, lembrete do atendimento com ${pastor?.nome||'o(a) pastor(a)'} em ${dv}/${m}/${y} às ${painel.hora}.`
                      waOpen(String(d.telefone||''), msg)
                    }} style={{ backgroundColor: '#25D366', color: '#fff' }} className="flex-1 py-2 rounded-lg text-sm font-semibold">📱 WA Fiel</button>
                    {pastor?.telefone && (
                      <button onClick={() => {
                        const d = painel.slot!.dados
                        const [y,m,dv] = painel.data.split('-')
                        const templateData = { nome: String(d.nome_fiel||''), nome_fiel: String(d.nome_fiel||''), pastor: pastor.nome, pastor_nome: pastor.nome, data: `${dv}/${m}/${y}`, hora: painel.hora, assunto: String(d.assunto||''), telefone: String(d.telefone||'') }
                        const msg = config?.msg_pastor ? preencherTemplate(config.msg_pastor, templateData) : `Pastor(a) ${pastor.nome}, lembrete: ${d.nome_fiel} em ${dv}/${m}/${y} às ${painel.hora}.`
                        waOpen(pastor.telefone, msg)
                      }} style={{ backgroundColor: '#128C7E', color: '#fff' }} className="flex-1 py-2 rounded-lg text-sm font-semibold">📱 WA Pastor</button>
                    )}
                  </div>
